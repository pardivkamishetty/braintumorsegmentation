...
hide_st_style = """
    ...
    .stApp {
        background-color: #8fc9e4; /* Light Gray - Main Background */
        padding: 20px;
    }
    ...
    if not st.session_state['logged_in']:
        ...
        # Upload Image page
            ...
            if uploaded_file is not None:
                ...
                # Run the prediction
                result_image, tumor_present, accuracy = predict_tumor(image, model)
        ...
        # Display accuracy
                accuracy = st.session_state['accuracy'] if 'accuracy' in st.session_state else "N/A"
                st.write(f"### Prediction Accuracy: {accuracy:.2f}%")
        
        # Display confidence level
                confidence = st.session_state['confidence'] if 'confidence' in st.session_state else "N/A"
